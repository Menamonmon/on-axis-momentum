...
class Planet:
    def __init__(self, name="earth", r=1, nphi=256, datDir=stdDatDir, info=True):
        ...
        self.datDir = datDir
        self.glm, self.hlm, self.lmax, self.idx = get_data(
            self.datDir, planet=self.name
        )

        self.p2D, self.th2D, self.Br, self.dipTheta, self.dipPhi = getBr(
            self, r=r, nphi=self.nphi, ntheta=self.ntheta, info=info
        )
    ...
    def plot(self, r=1, levels=30, cmap="RdBu_r", proj="Mollweide"):
        ...
        if r == 1:
            ax, cbar = plotSurf(
                self.p2D, self.th2D, self.Br, levels=levels, cmap=cmap, proj=proj
            )
        else:
            self.r = r
            self.p2D, self.th2D, self.Br, self.dipTheta, self.dipPhi = getBr(
                self, r=r, nphi=self.nphi, ntheta=self.ntheta, info=True)
            ax, cbar = plotSurf(
                self.p2D, self.th2D, self.Br, levels=levels, cmap=cmap, proj=proj
            )
    ...
    def writeVtsFile(self, potExtra=False, ratio_out=2, nrout=32):
        ...
        if potExtra:
            brout, btout, bpout = extrapot(self.lmax, 1.0, self.Br, rout)
        else:
            brout = self.Br
    ...
    def plot_filt(
        ...
        if self.name in ["mercury", "saturn"]:
            self.glm_filt, self.hlm_filt = filt_Gaussm0(
                self.glm,
                self.hlm,
                self.lmax,
                larr=self.larr_filt,
                lCutMin=self.lCutMin,
                lCutMax=self.lCutMax,
            )
            self.Br_filt = 1e-3 * getBm0(
                self.lmax, self.glm_filt, self.r_filt, self.p2D, self.th2D
            )
        else:
            self.glm_filt, self.hlm_filt = filt_Gauss(
                self.glm,
                self.hlm,
                self.lmax,
                self.idx,
        ...
        ax, cbar = plotSurf(
            self.p2D, self.th2D, self.Br_filt, levels=levels, cmap=cmap, proj=proj
        )
    ...
    def spec(self, r=1, iplot=True):
        self.emag_spec, emag_10 = get_spec(
            self.glm, self.hlm, self.idx, self.lmax, planet=self.name, r=r
        )
